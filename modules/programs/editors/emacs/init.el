...
(use-package lin
  ;; The `lin' package is a stylistic enhancement for Emacs' built-in
  ...
  ;;
  ;; The idea is that `hl-line-mode' cannot work equally well for contexts with
  ;; competing priorities: (i) line selection, or (ii) simple line highlight. In
  ;; the former case, the current line needs needs to be made prominent because
  ;; it carries a specific meaning of some significance in the given context:
  ;; the user has to select a line. Whereas in the latter case, the primary mode
  ;; of interaction does not revolve around the line highlight itself: it may be
  ;; because the focus is on editing text or reading through the buffer's
  ;; contents, so the current line highlight is more of a reminder of the
  ;; point's location on the vertical axis.
  :config
...
(use-package exec-path-from-shell
  :config
  ;; A macOS GUI Emacs is launched by the window server rather than from a login
  ;; shell, so it does not inherit the shell's environment -- most importantly
  ;; `PATH'. This package runs the shell once at startup and copies the
  ;; resulting environment into Emacs. It is only needed for the GUI build on
  ;; macOS; terminal Emacs and Emacs on Linux already inherit the shell
  ;; environment.
  (when (memq window-system '(mac ns))
    (setopt exec-path-from-shell-shell-name "/bin/bash")
...
(use-package dired
  ...
  ;;
  ;; '--time-style=long-iso'
  ;; Uses the international standard for time representation in the file
...
(use-package minibuffer
  ...
  (setopt completion-styles '(basic substring initials partial-completion flex))
  (setopt completion-flex-nospace t)
  (setopt completion-pcm-leading-wildcard nil) ; Emacs 31
  (with-eval-after-load 'orderless
    (setopt completion-styles (append completion-styles '(orderless))))
  ...
  (setopt minibuffer-history-case-insensitive-variables
          '(minibuffer-history
  ...
  ;; You can use the arrow keys to select completion candidates in the
  ;; minibuffer, and you can use `RET' to select the highlighted one.
  (setopt minibuffer-visible-completions t)

  ;; Additional details for completion suggestions are shown before or
  ;; after the suggestions. For example `describe-symbol' (`C-h\ o')
  ;; shows additional information.
  (setopt completions-detailed t)

  ;; Completion candidates can be grouped together if the function that
  ;; sets up the completion specifies it.
  (setopt completions-group t)

  ;; When you press `TAB' to see the completion candidates for a prompt
  ;; (for example, `M-x' and then `TAB'), the first `TAB' will display
  ;; the completion list, and the second `TAB' will select the buffer.
  (setopt completion-auto-select 'second-tab)

  ;; This Completions buffer will update as you type so that you can
  ;; narrow down the candidates.
  (setopt completion-eager-update t)

  ;; When completing file names, pressing `DEL' at a directory separator
  ;; goes up one directory instead of deleting a single character.
  ;; Pressing `M-DEL' does the same at a separator, or kills a word
  ;; otherwise.  This is similar to how `vertico-directory' and
  ;; `icomplete-fido-backward-updir' behave.
  (defun +minibuffer-backward-updir (&optional n)
  ...
  (defun +minibuffer-delete-char (n)
    "Delete N directories or chars before point."
    (interactive "p")
    (unless (+minibuffer-backward-updir n)
      (backward-delete-char n)))

  (defun +minibuffer-delete-word (n)
    "Delete N directories or words before point."
    (interactive "p")
    (unless (and (eq (char-before) ?/) (+minibuffer-backward-updir n))
      (backward-kill-word n)))

  (bind-keys :map minibuffer-local-filename-completion-map
             ("DEL" . +minibuffer-delete-char)
             ("M-DEL" . +minibuffer-delete-word)))
...
(use-package orderless
  ...
  (setopt orderless-smart-case nil)
...
first one. Else do `vertico-exit'."
  ...
  (bind-keys :map vertico-map
             ...
             :map ctl-x-map
             ("z" . vertico-repeat))) ; C-x z, orig. `repeat'
...
;; TODO: snippets -- svitax used `tempel' (integrated via `cape'); both were dropped
;; here. Revisit with tempel or yasnippet later.

;; TODO: abbrev -- not configured (svitax had only a commented `(use-package abbrev)'
;; anchor). Revisit `abbrev-mode' / Prot's abbrev tables later.
...
(use-package dape
  ...
  :config
  (bind-key "y" dape-global-map ctl-x-map)
        ...
        (progn
          ...
          (when +elisp-better-lisp-indent
            (advice-add 'calculate-lisp-indent
                        :override #'+elisp-calculate-lisp-indent)))
...
`python-nav-end-of-statement'."
  ...
  (bind-keys :map python-ts-mode-map
             ...
             ("C-c C-q" . kill-current-buffer) ;; TODO: ("C-c C-q" . +python-shell-quit)
...
there."
  ...
  (bind-keys
   :map ctl-x-map
   ("c" . org-capture)
   ("i" . +org-capture-inbox) ; orig. `insert-file'
...
continue, per `org-agenda-skip-function'."
  ...
  (bind-keys
   ...
   ("a" . +org-agenda-custom) ; orig. abbrev prefix
   ("C-a" . org-agenda)
...
`.csl'."
  ...
  (defvar-keymap +bib-prefix-map
    :doc "Prefix keymap for bibliography commands."
    :prefix '+bib-prefix-map)
  (bind-keys :map ctl-x-map
             ("l" . +bib-prefix-map)) ; orig. count-lines-page
...
NO-DISPLAY is sent forward to `notmuch-search'."
  ...
  (bind-keys :map ctl-x-map
             ("m" . notmuch-mua-new-mail) ; orig. `compose-mail'
