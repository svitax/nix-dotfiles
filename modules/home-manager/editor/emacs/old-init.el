...
;; Load no-littering early.
(use-package no-littering
  :ensure t
  :init
  (defalias 'etc #'no-littering-expand-etc-file-name)
  (defalias 'var #'no-littering-expand-var-file-name))
...
(use-package bind
  ...
  (let ((tail (nthcdr (cl-position :init use-package-keywords)
    ...
    (setcdr tail (cons :bound (cdr tail))))

  (defun use-package-normalize/:bound (name-symbol keyword args)
    "Normalizer for `:bound' keyword used in `use-package' forms."
  ...
  (defun use-package-handler/:bound (name-symbol keyword form rest state)
...
(use-package meow-extras
  ...
  (keymap-set +goto-prefix-map "b" '+meow-window-bottom)
  (keymap-set +goto-prefix-map "c" '+meow-window-middle)
  (keymap-set +goto-prefix-map "t" '+meow-window-top))
...
(use-package files
  ...
  :init (keymap-set +file-prefix-map "f" #'find-file))
...
(use-package consult
  ...
  :bind (:map +buffer-prefix-map
	 ("C-b" . consult-buffer)
	 ;; TODO: should i use M- , C- , or S- for bindings in my meow keypad maps
	 ("M-f" . consult-focus-lines) ;; idk
	 ("M-k" . consult-keep-lines)
	 ("M-y" . consult-yank-pop)
	 :map +search-prefix-map
	 ;; TODO: so . consult-multi-occur (lambda emacs)
	 ;; TODO: s. . consult-line-symbol-at-point (lambda emacs)
	 ("C-g" . consult-ripgrep)
	 ("g" . consult-grep)
	 ...
	 ("C-l" . consult-outline)
	 :map +file-prefix-map
	 ;; TODO: fy . my/copy-buffer-filename
	 ("i" . consult-fd)
	 ("l" . consult-locate)
...
(use-package consult-extras
  :bind ( :map +search-prefix-map
	       ("C-i" . my/consult-imenu-all)
	       ("C-k" . my/consult-mark-all)))
...
(use-package project
  :bind (:map +file-prefix-map
	 ("C-f" . project-find-file)
	 :map +project-prefix-map
	 ("b" . project-switch-to-buffer)
	 ("d" . project-dired)
	 ("e" . project-eshell)
	 ("f" . project-find-file) ;; can i replace this with consult-dir-jump-file?
  ...
  (project-switch-commands '((?b "Buffer" project-switch-to-buffer)
...
(use-package buffers-extras
  :config
  (keymap-set +goto-prefix-map "e" 'end-of-buffer)
  ;; gf => goto file
  (keymap-set +goto-prefix-map "g" 'beginning-of-buffer)
  (keymap-set +buffer-prefix-map "C-k" 'kill-current-buffer)
  (keymap-set +buffer-prefix-map "C-s" 'save-buffer))
...
(use-package windows-extras
  ...
  :bind (("M-o" . other-window)
	 ...
	 ("s" . split-window-below) ;; TODO: my/split-window-below-and-focus (lambda emacs)
	 ;; wr . my/rotate-windows (lambda emacs)
	 ;; wR . my/rotate-windows-backward (lambda emacs)
	 ;; wx . my/exchange-window-buffer (lambda emacs)
	 ("o" . other-window)
	 ("k" . delete-window)
	 ;; ("t" . +transpose-windows) ;; swaps buffers in current and next window
	 ("T" . tear-off-window)
...
(use-package editing-extras
  ...
  :config
  (keymap-set +goto-prefix-map "i" 'end-of-line) ;; TODO: replace with mosey or alternative
  (keymap-set +goto-prefix-map "m" 'beginning-of-line) ;; TODO replace with mosey or alternative
...
(use-package xref
  ...
  :config
  (keymap-set +goto-prefix-map "d" 'xref-find-definitions)
  (keymap-set +goto-prefix-map "r" 'xref-find-references)
  ;; gD => goto declaration
...
(use-package git-gutter-fringe
  ...
  ;; TODO: eventually have a :meow-bind use-package keyword that doesn't add autoloads
  (keymap-set +vc-prefix-map "n" #'git-gutter:next-hunk)
  (keymap-set +vc-prefix-map "e" #'git-gutter:previous-hunk))
...
;; org
;; TODO: org format or whitespace butler
(use-package org
  :custom
...
(use-package citar
  ...
  :bind (:map +bibliography-prefix-map
	      ("a" . citar-denote-add-citekey)
	      ("b" . citar-open)
	      ("c" . citar-create-note)
	      ("d" . citar-denote-dwim)
	      ("f" . citar-open-files)
	      ("r" . citar-denote-remove-citekey)
	      ("n" . citar-open-notes))
  :custom
...
(use-package pdf-tools
  ...
  (pdf-view-mode . (lambda () (progn
				...
				(hl-line-mode -1)
				(keymap-set pdf-view-mode-map "n" 'pdf-view-next-line-or-next-page)
				(keymap-set pdf-view-mode-map "e" 'pdf-view-previous-line-or-previous-page)
				(keymap-set pdf-view-mode-map "C-n" 'pdf-view-next-page-command)
				(keymap-set pdf-view-mode-map "C-e" 'pdf-view-previous-page-command))))
