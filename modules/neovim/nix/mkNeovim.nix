...
withPython3 ? true, withSqlite ? true, withRuby ? false, withNodeJs ? false
, viAlias ? true, vimAlias ? true, neovim-src, }:
let
  defaultPlugin = {
  ...
  '' + (builtins.readFile ../nvim/init.lua) + neovimConfig.neovimRcContent
...
in pkgs.wrapNeovimUnstable (pkgs.neovim-unwrapped.overrideAttrs (_: {
  # Use Neovim nightly
  src = neovim-src;
